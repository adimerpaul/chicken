    ...
    <q-dialog v-model="dlg" persistent>
      <q-card style="width: 520px; max-width: 90vw">
        <q-card-section class="q-pb-none row items-center">
          <div>{{ item.id ? 'Editar' : 'Nuevo' }} almacén</div>
          <q-space/>
...
export default {
  ...
  methods: {
    async updateStock (row) {
      if (!row.id) return
      try {
        await this.$axios.put(`almacenes/${row.id}`, {
          stock: row.stock
        })
    ...
    async onDelete (id) {
      this.$alert.dialog('¿Eliminar el almacén?')
        .onOk(async () => {
          this.loading = true
          try {
            await this.$axios.delete(`almacenes/${id}`)
    ...
    async submit () {
      ...
      try {
        if (this.item.id) {
          await this.$axios.put(`almacenes/${this.item.id}`, this.item)
          this.$alert?.success?.('Actualizado')
        } else {
          await this.$axios.post('almacenes', this.item)
